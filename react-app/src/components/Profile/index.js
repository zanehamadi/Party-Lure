...
export default function Profile({ users, parties, roles, jobs }) {
    ...
    const receivedRequestState = useSelector(state => state.requests.received)

    let sentRequests = Object.values(sentRequestState)
    let receivedRequests = Object.values(receivedRequestState)
            ...
            {owner &&
                <div>
                    ...
                    {receivedRequests && receivedRequests.map(req => {
                        return (
                            <ProfileReceivedRequests key={req.id} partyId={req.id} requests={req.requests} />
                        )
                    })} */}
                    <h2>Sent Requests</h2>
